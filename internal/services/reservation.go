...
func SuggestAlternativeDates(conf *configuration.Dependencies, hallID uint, requestedStart, requestedEnd time.Time) ([]DateRange, error) {
	...
	var suggestions []DateRange
	requestedDuration := requestedEnd.Sub(requestedStart)

	// If no reservations exist, suggest the requested dates as available.
	if len(reservations) == 0 {
		suggestions = append(suggestions, DateRange{Start: requestedStart, End: requestedStart.Add(requestedDuration)})
		return suggestions, nil
	}

	// Check gap before the first reservation.
	if reservations[0].StartDate.Sub(startWindow) >= requestedDuration {
		suggestions = append(suggestions, DateRange{Start: startWindow, End: startWindow.Add(requestedDuration)})
	}
	...
	for i := 0; i < len(reservations)-1; i++ {
		...
		if gap >= requestedDuration {
			suggestions = append(suggestions, DateRange{
